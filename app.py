...
# ──────────────────────────────────────────────
#  Widget 個人設定儲存（PostgreSQL）
# ──────────────────────────────────────────────
import psycopg2
from psycopg2.extras import RealDictCursor

_db_url = os.environ.get("DATABASE_URL", "")
_widget_configs: dict = {}  # fallback when no DB
        ...
        try:
            with _get_conn() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    cur.execute("SELECT station, direction FROM widget_configs WHERE key=%s", (key,))
                    row = cur.fetchone()
            if not row:
                return jsonify({"error": "找不到設定"}), 404
            return jsonify({"station": row["station"], "direction": row["direction"]})
        except Exception as e:
    ...
    if _db_url:
        try:
            with _get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        INSERT INTO widget_configs (key, station, direction)
                        VALUES (%s, %s, %s)
                        ON CONFLICT (key) DO UPDATE
                        SET station=EXCLUDED.station, direction=EXCLUDED.direction
